...
function text_setting(key, data) {
    ...
    }else if(key.includes("api_cost_saving")){
...
function set_option(func, obj_data, obj, obj_option, obj_mapping, dataZoom) {
    ...
    if (func){
        if (func["type"] === "option"){
            obj.hideLoading();
            obj.setOption(func["func"](obj_data));
        }else if (func["type"] === "p_function"){
            func["func"](obj_data);
        }
    }else{
...
function data_render(request_data, time_ids, funcs=[false], chart_objs=[false], option_datas=[false], option_mappings=[false], dataZooms=[1], request_url=kamba_url, request_type='POST'){
    // 数据等待
    try {
        for (let i=0;i<chart_objs.length;i++){
            if (chart_objs[i]){
                chart_objs[i].showLoading();
            }
        }
        $.ajax(
            {
                ...
                success: function (data) {
                    ...
                    for (let i=0;i<chart_objs.length;i++){
                        // 时间设定 按照data返回值中的start及end
                        set_time_range(time_ids[i], data);
